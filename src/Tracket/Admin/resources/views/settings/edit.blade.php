...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data">
        ...
        <div class="pb-5 mb-0 sm:flex sm:items-center sm:justify-between">
            <h3 class="text-2xl leading-6 font-medium text-gray-900">Update Settings</h3>
            <div class="mt-3 flex sm:mt-0 sm:ml-4">
                <button type="submit" class="ml-3 inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Save</button>
            </div>
        </div>
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-4 pb-4 sm:grid-cols-6">
                    @foreach ($settings as $setting)
                        <div class="sm:col-span-4">
                            @include("admin::settings.types.{$setting->getType()}", [
                                'setting' => $setting
                            ])
                        </div>
                    @endforeach
